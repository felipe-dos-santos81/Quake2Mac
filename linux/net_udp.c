...
// the client spins at ~1000 fps (no vsync block) and sends a packet
// every frame, while the server only drains this queue at ~10 Hz; with
// the original depth of 4, one-shot reliable packets (connect, new,
// begin) were overwritten before the server could read them.  Must
// stay a power of two.
