...
*/
// user.c -- server code for moving users
...
void SV_Begin_f (void)
{
	...
	// handle the case of a level changing while a client was connecting;
	// demo servers stream a recorded handshake whose spawncount belongs to
	// the original server, so it can never match ours
	if (sv.state != ss_demo && atoi(Cmd_Argv(1)) != svs.spawncount )
	{
	...
	// a demo client joined while the handshake was streaming; restart the
	// frame stream at its first frame so it receives the entity delta
	// chain from the beginning instead of mid-chain
	if (sv.state == ss_demo && sv.demofile && sv.demo_frame_pos)
		fseek (sv.demofile, sv.demo_frame_pos, SEEK_SET);

	// call the game begin function; demo clients have no edict
	if (sv.state != ss_demo)
