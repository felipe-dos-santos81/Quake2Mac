...
*/
// send.c -- server message output to clients
...
void SV_SendClientMessages (void)
{
	...
	if (sv.state == ss_demo && sv.demofile)
	{
		...
		else
		{
			...
			// remember where the frame stream starts; SV_Begin_f rewinds
			// here so a client receives the delta chain from its first
			// frame, not mid-chain
			if (!sv.demo_frame_pos && msglen > 0 && msgbuf[0] == svc_frame)
				sv.demo_frame_pos = savepos;
		}
	}
	...
	for (i=0, c = svs.clients ; i<maxclients->value; i++, c++)
	{
		...
		{
			// hold frames back until the client has finished precaching
			if (sv.state == ss_demo && c->state != cs_spawned
				&& msglen > 0 && msgbuf[0] == svc_frame)
			{
