...
- **Walk/look mouse scheme (2026-09-03):** look is the default; right click or the HUD `WALK`/`LOOK` button toggles the archived `walkmode` cvar, switching `RW_IN_Move` axes from angles to movement and leveling pitch on entry; left double-click jumps. `RW_IN_CursorPos` (cursor is never grabbed) is part of the ref export contract. `+mlook`/`freelook`/`lookstrafe` are deliberate no-ops kept for old configs, and the wheel's weapon-switch binds are deliberately alive — do not "clean" either.
