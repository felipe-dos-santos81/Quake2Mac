...
*/
// scrn.c -- master for refresh, status bar, console, chat, notify, etc
...
// the finale prints the characters one at a time
	...
	// scan the width of the line
		for (l=0 ; l<40 ; l++)
			if (start[l] == '\n' || !start[l])
				break;
		x = (viddef.width - l*cw)/2;
		SCR_AddDirtyPoint (x, y);
		for (j=0 ; j<l ; j++, x+=cw)
		{
			re.DrawStretchChar (x, y, cw, cw, start[j]);
			if (!remaining--)
				return;
		}
		SCR_AddDirtyPoint (x, y+cw);
...
HUD scale factor. scr_hudscale 0 (default) scales the HUD
proportionally to the screen size, keeping the same ratios
as at 640x480. Positive values override the factor.
================
*/
float SCR_HUDScale (void)
{
	...
	if (scr_hudscale->value > 0)
		scale = scr_hudscale->value;
	else
		scale = viddef.height / 480.0;
...
void SCR_Init (void)
{
	...
	Cvar_Get ("scr_showturtle", "0", 0);
	scr_showpause = Cvar_Get ("scr_showpause", "1", 0);
	scr_centertime = Cvar_Get ("scr_centertime", "2.5", 0);
	Cvar_Get ("scr_printspeed", "8", 0);
	...
	scr_hudscale = Cvar_Get ("scr_hudscale", "0", CVAR_ARCHIVE);
	...
	float scale)
{
	...
	while (*string)
	{
		...
		for (i=0 ; i<width ; i++)
		{
			re.DrawStretchChar (x, y, cw, cw, line[i]^xor);
			x += cw;
		}
		if (*string)
		{
...
void SCR_DrawField (int x, int y, int color, int width, int value, float scale)
{
	...
	SCR_AddDirtyPoint (x, y);
	SCR_AddDirtyPoint (x+width*CHAR_WIDTH*scale+2, y+23*scale);

	Com_sprintf (num, sizeof(num), "%i", value);
	l = strlen(num);
	if (l > width)
		l = width;
	x += 2 + CHAR_WIDTH*scale*(width - l);
...
void SCR_ExecuteLayoutString (char *s)
{
	...
	while (s)
	{
		...
		{
			token = COM_Parse (&s);
			y = viddef.height/2 - 120*scale + atoi(token)*scale;
			continue;
		}
		...
		{	// draw a ctf client block
			...
			if (value == cl.playernum)
				SCR_DrawAltStringScaled (x, y, block, scale);
			else
				SCR_DrawStringScaled (x, y, block, scale);
			continue;
		}
		...
		if (!strcmp(token, "stat_string"))
		{
			...
			if (index < 0 || index >= MAX_CONFIGSTRINGS)
				Com_Error (ERR_DROP, "Bad stat_string index");
			SCR_DrawStringScaled (x, y, cl.configstrings[index], scale);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawHUDString (token, x, y, 320, 0, scale);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			SCR_DrawStringScaled (x, y, token, scale);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawHUDString (token, x, y, 320, 0x80, scale);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			SCR_DrawAltStringScaled (x, y, token, scale);
			continue;
		}
