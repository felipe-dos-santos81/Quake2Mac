...
void CL_Precache_f (void)
{
	...
	if (Cmd_Argc() < 2) {
		...
		// demo servers stream old recordings whose precache command
		// carries no spawncount; confirm the spawn anyway so the
		// server starts delivering frames
		MSG_WriteByte (&cls.netchan.message, clc_stringcmd);
		MSG_WriteString (&cls.netchan.message, va("begin %i\n", cl.servercount) );
		return;
	}
